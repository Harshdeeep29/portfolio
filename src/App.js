...
const App = () => {
    ...
    // Effect to update active section based on scroll position for navigation highlighting
    useEffect(() => {
        const handleScroll = () => {
            const sections = ['profile', 'education', 'skills', 'projects', 'experience', 'certifications'];
            let currentActive = 'profile'; // Default to profile

            // Get the height of the fixed navigation bar
            const navBar = document.querySelector('nav');
            // Fallback to a default height if navBar is null (e.g., during initial render)
            const navBarHeight = navBar ? navBar.offsetHeight : 64; // Assuming ~64px for p-4 (16*4) nav
            ...
            for (let i = sections.length - 1; i >= 0; i--) {
                ...
                if (element) {
                    ...
                    // A section is active if its top is above a certain threshold (e.g., navBarHeight + some buffer)
                    // and its bottom is still visible. This makes sure the active section is visible below the nav.
                    if (rect.top <= navBarHeight + 50 && rect.bottom > navBarHeight) { // Added 50px buffer
    ...
    }, [isMenuOpen]); // Added isMenuOpen to dependency array to re-evaluate on menu state change
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-gray-100 font-inter">
            ...
            <nav className="fixed top-0 left-0 right-0 bg-gray-900 bg-opacity-90 backdrop-blur-sm shadow-lg z-50 p-4">
                <div className="container mx-auto flex justify-between items-center">
                    ...
                    <div className="md:hidden">
                        <button
                            ...
                            className="text-gray-300 hover:text-blue-300 focus:outline-none focus:text-blue-300"
                    ...
                    {/* Navigation Links - Hidden on small, visible on medium and up */}
                ...
                {isMenuOpen && (
                    <div className="md:hidden mt-4 bg-gray-900 bg-opacity-90 backdrop-blur-sm pb-4">
                                ...
                                <li key={section}>
                                    <button
                                        onClick={() => {
                                            ...
                                            // setIsMenuOpen(false); // This is now handled in scrollToSection
                                        }}
                                        className={`text-xl font-medium transition-colors duration-300 ${activeSection === section ? 'text-blue-400' : 'text-gray-300 hover:text-blue-300'}`}
                                    >
                                        {section.charAt(0).toUpperCase() + section.slice(1)}
                                    </button>
                                </li>
            ...
            <header id="profile" className="relative h-screen flex items-center justify-center text-center overflow-hidden p-4 sm:p-6 md:p-8">
                ...
                {/* Added pt-16 to ensure content starts below the fixed navigation bar */}
                <div className="relative z-10 p-6 sm:p-8 md:p-10 rounded-lg shadow-2xl bg-gray-800 bg-opacity-70 max-w-sm sm:max-w-md md:max-w-3xl lg:max-w-4xl mx-auto transform transition-all duration-500 hover:scale-105 pt-16">
                    {/* Adjusted base text sizes for better mobile responsiveness */}
                    <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-blue-400 mb-3 sm:mb-4 animate-fade-in-up">{resumeData.name}</h2>
                    <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-light text-gray-200 mb-4 sm:mb-6 animate-fade-in-up delay-200">{resumeData.title}</p>
                    <p className="text-sm sm:text-base md:text-lg lg:text-xl text-gray-300 leading-relaxed animate-fade-in-up delay-400">{resumeData.profile}</p>
                    <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row justify-center space-y-4 sm:space-y-0 sm:space-x-6 animate-fade-in-up delay-600">
                        <a href={`mailto:${resumeData.contact.email}`} className="flex items-center justify-center space-x-2 px-5 py-2 sm:px-6 sm:py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg transition-transform transform hover:-translate-y-1 text-sm sm:text-base">
                        ...
                        <a href={resumeData.contact.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center space-x-2 px-5 py-2 sm:px-6 sm:py-3 bg-gray-700 hover:bg-gray-600 text-white rounded-full shadow-lg transition-transform transform hover:-translate-y-1 text-sm sm:text-base">
                        ...
                        <a href={resumeData.contact.github} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center space-x-2 px-5 py-2 sm:px-6 sm:py-3 bg-gray-700 hover:bg-gray-600 text-white rounded-full shadow-lg transition-transform transform hover:-translate-y-1 text-sm sm:text-base">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" viewBox="0 0 24 24" fill="currentColor">
            ...
            <main className="container mx-auto px-4 sm:px-6 py-12 sm:py-16"> {/* Adjusted main padding */}
                {/* Education Section */}
                <section id="education" className="mb-16 p-6 sm:p-8 bg-gray-800 rounded-xl shadow-xl transform transition-all duration-300 hover:scale-[1.02]">
                    <h3 className="text-3xl sm:text-4xl font-bold text-blue-400 mb-6 text-center">Education</h3>
                    <div className="flex flex-col md:flex-row items-center md:space-x-8">
                        <div className="md:w-1/3 mb-6 md:mb-0">
                            {/* Increased image max-width for better responsiveness on larger screens */}
                            <img src="https://mucdoe.samarth.ac.in/uploads/uims/c9c707794a80127622878c8a2659e2b629aa319b34f98a1644c1375e677d19a71_1739442624_53547382_logo.png" alt="University Logo" className="rounded-lg shadow-lg mx-auto max-w-xs sm:max-w-sm md:max-w-full" />
                        </div>
                        <div className="md:w-2/3 text-center md:text-left">
                            <p className="text-xl sm:text-2xl md:text-3xl font-semibold text-gray-200">{resumeData.education.degree}</p>
                            <p className="text-lg sm:text-xl text-gray-300 mt-2">{resumeData.education.university}</p>
                            <p className="text-md sm:text-lg text-gray-400 mt-1">{resumeData.education.years}</p>
                            <p className="text-md sm:text-lg text-gray-400 mt-1">GPA: {resumeData.education.gpa}</p>
                        </div>
                    </div>
                </section>

                {/* Skills Section */}
                <section id="skills" className="mb-16 p-6 sm:p-8 bg-gray-800 rounded-xl shadow-xl transform transition-all duration-300 hover:scale-[1.02]">
                    <h3 className="text-3xl sm:text-4xl font-bold text-blue-400 mb-8 text-center">Technical Skills</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8"> {/* Adjusted gap */}
                        {Object.entries(resumeData.skills).map(([category, skills]) => (
                            <div key={category} className="bg-gray-700 p-6 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300">
                                <h4 className="text-xl sm:text-2xl font-semibold text-gray-100 mb-4 capitalize">{category.replace(/([A-Z])/g, ' $1').trim()}</h4>
                                <ul className="list-disc list-inside text-gray-300 space-y-2">
                                    {skills.map((skill, index) => (
                                        <li key={index} className="flex items-center text-base sm:text-lg"> {/* Adjusted text size */}
                ...
                <section id="experience" className="mb-16 p-6 sm:p-8 bg-gray-800 rounded-xl shadow-xl transform transition-all duration-300 hover:scale-[1.02]">
                    <h3 className="text-3xl sm:text-4xl font-bold text-blue-400 mb-8 text-center">Work Experience</h3>
                    <div className="space-y-6 sm:space-y-8"> {/* Adjusted space */}
                        {resumeData.experience.map((job, index) => (
                            <div key={index} className="bg-gray-700 p-6 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300">
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2">
                                    <h4 className="text-xl sm:text-2xl font-semibold text-gray-100">{job.title}</h4>
                                    <span className="text-gray-400 text-sm sm:text-md mt-1 sm:mt-0">{job.duration}</span> {/* Adjusted text size and margin */}
                                </div>
                                <p className="text-blue-300 text-base sm:text-lg mb-4">{job.company}</p>
                                <ul className="list-disc list-inside text-gray-300 space-y-2 text-base sm:text-lg"> {/* Adjusted text size */}
                ...
                <section id="certifications" className="mb-16 p-6 sm:p-8 bg-gray-800 rounded-xl shadow-xl transform transition-all duration-300 hover:scale-[1.02]">
                    <h3 className="text-3xl sm:text-4xl font-bold text-blue-400 mb-8 text-center">Certifications</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6"> {/* Adjusted gap */}
                        {resumeData.certifications.map((cert, index) => (
                            <div key={index} className="bg-gray-700 p-6 rounded-lg shadow-md text-center hover:shadow-xl transition-shadow duration-300">
                                <p className="text-base sm:text-xl font-medium text-gray-100">{cert}</p> {/* Adjusted text size */}
                            </div>
                        ))}
                    </div>
                </section>
            ...
            {/* Footer */}
            <footer className="bg-gray-900 py-6 sm:py-8 text-center text-gray-400 text-sm sm:text-base"> {/* Adjusted padding and text size */}
                <p>&copy; {new Date().getFullYear()} {resumeData.name}. All rights reserved.</p>
                <div className="flex justify-center space-x-3 sm:space-x-4 mt-3 sm:mt-4 text-sm sm:text-base"> {/* Adjusted spacing and text size */}
